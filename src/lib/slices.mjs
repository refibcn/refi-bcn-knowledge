...
// rows a viewmodel already produced) — no store read, no JSON read, no
// dependency of its own on where the data came from. That is deliberate, not
// an oversight: the page that renders `/slices` decides what feeds these
// functions, this module just computes. If a future edit here wants to call
// `loadKb` or read a JSON file itself, that is a sign the code belongs in
// the page, not in this module.
//
...
// sense. That is fine, not a loophole: every other consumer that touches the
// store (`collections.mjs`, `sources.mjs`) already pays that same load-time
// walk, and anything that imports this module in a real build pulls those
// siblings in too.
//
// `highRiskQueue`'s throw-on-absent follows the doctrine already stated in
// archive-ready.mjs: a missing `unresolved_high_risk` is a
 ...
 * `by_maturity` rollup (the shape `sourceContainers` produces).
 *
    ...
    if (r.unresolved_high_risk === undefined) {
      throw new Error(
        `slices.mjs: highRiskQueue — "${r.id}" carries no unresolved_high_risk count. ` +
          ...
          "Rows must come from sourcesViewModel().rows, not be hand-constructed.",
      );
    }
